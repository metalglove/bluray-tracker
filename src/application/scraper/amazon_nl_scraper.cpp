...
std::optional<domain::Product> AmazonNlScraper::scrape(std::string_view url) {
  ...
  Logger::instance().info(fmt::format("Scraping Amazon.nl: {}", url));

  // Fetch HTML
  auto response = client_.get(url);
  if (!response.success) {
    Logger::instance().error(
        fmt::format("Failed to fetch Amazon.nl page: {} (status: {})", url,
...
std::optional<AmazonNlScraper::ScrapedData>
AmazonNlScraper::parseHtml(const std::string &html) {
...
std::optional<std::string> AmazonNlScraper::extractTitle(GumboNode *root) {
  ...
  const char *title_ids[] = {"productTitle", "title", "btAsinTitle"};
...
std::optional<double> AmazonNlScraper::extractPrice(GumboNode *root) {
  ...
  const char *price_classes[] = {"a-price-whole", "a-offscreen", "a-price"};
...
bool AmazonNlScraper::extractStockStatus(GumboNode *root) {
  ...
  const char *out_of_stock_classes[] = {"availability", "a-color-price",
                                        "a-color-error"};
...
bool AmazonNlScraper::extractUhdStatus(GumboNode *root,
...
GumboNode *AmazonNlScraper::findElementById(GumboNode *node, const char *id) {
  ...
  for (unsigned int i = 0; i < children->length; ++i) {
    if (auto *result =
            findElementById(static_cast<GumboNode *>(children->data[i]), id)) {
      return result;
    }
  }
...
GumboNode *AmazonNlScraper::findElementByClass(GumboNode *node,
...
AmazonNlScraper::findElementsByClass(GumboNode *node, const char *class_name) {
