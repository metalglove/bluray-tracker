...
std::optional<domain::Product> BolComScraper::scrape(std::string_view url) {
    ...
    Logger::instance().info(std::format("Scraping Bol.com: {}", url));

    // Fetch HTML
    auto response = client_.get(url);
    if (!response.success) {
        Logger::instance().error(
            std::format("Failed to fetch Bol.com page: {} (status: {})",
...
std::optional<BolComScraper::ScrapedData> BolComScraper::parseHtml(const std::string& html) {
    ...
    for (const char* class_name : price_classes) {
        ...
        for (auto* node : nodes) {
            if (node->type == GUMBO_NODE_ELEMENT) {
                ...
                std::function<std::string(GumboNode*)> getText = [&](GumboNode* n) -> std::string {
...
bool BolComScraper::extractUhdStatus(GumboNode* root, const std::string& title) {
        ...
        if (auto* node = findElementByClass(root, class_name)) {
            ...
            if (node->type == GUMBO_NODE_ELEMENT) {
                if (auto* img = findElementByTag(node, GUMBO_TAG_IMG)) {
                    GumboAttribute* src_attr = gumbo_get_attribute(&img->v.element.attributes, "src");
                    if (src_attr && src_attr->value) {
                        return std::string(src_attr->value);
                    }
                }
...
GumboNode* BolComScraper::findElementByClass(GumboNode* node, const char* class_name) {
    if (node->type != GUMBO_NODE_ELEMENT) {
        return nullptr;
    }
...
std::vector<GumboNode*> BolComScraper::findElementsByClass(GumboNode* node, const char* class_name) {
...
GumboNode* BolComScraper::findElementByTag(GumboNode* node, GumboTag tag) {
    ...
    GumboVector* children = &node->v.element.children;
    for (unsigned int i = 0; i < children->length; ++i) {
        if (auto* result = findElementByTag(static_cast<GumboNode*>(children->data[i]), tag)) {
            return result;
        }
    }
