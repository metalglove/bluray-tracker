...
int SqliteCollectionRepository::add(const domain::CollectionItem& item) {
    ...
    sqlite3_bind_text(stmt.get(), 9, purchased_at_str.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt.get(), 10, added_at_str.c_str(), -1, SQLITE_TRANSIENT);

    if (sqlite3_step(stmt.get()) != SQLITE_DONE) {
        Logger::instance().error(
            std::format("Failed to insert collection item: {}", sqlite3_errmsg(db.getHandle()))
        );
        return -1;
    }
...
bool SqliteCollectionRepository::update(const domain::CollectionItem& item) {
    ...
    sqlite3_bind_text(stmt.get(), 4, item.image_url.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt.get(), 5, item.local_image_path.c_str(), -1, SQLITE_TRANSIENT);
    ...
    sqlite3_bind_text(stmt.get(), 8, purchased_at_str.c_str(), -1, SQLITE_TRANSIENT);
...
std::optional<domain::CollectionItem> SqliteCollectionRepository::findById(int id) {
    auto& db = DatabaseManager::instance();
    auto lock = db.lock();
...
std::optional<domain::CollectionItem> SqliteCollectionRepository::findByUrl(std::string_view url) {
    auto& db = DatabaseManager::instance();
    auto lock = db.lock();
...
domain::PaginatedResult<domain::CollectionItem> SqliteCollectionRepository::findAll(
...
domain::CollectionItem SqliteCollectionRepository::fromStatement(sqlite3_stmt* stmt) {
    ...
    if (const char* notes = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 8))) {
        item.notes = notes;
    }

    const char* purchased_at = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 9));
    const char* added_at = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 10));
...
std::chrono::system_clock::time_point SqliteCollectionRepository::stringToTimePoint(
