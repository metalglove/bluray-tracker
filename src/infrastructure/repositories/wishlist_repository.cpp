...
int SqliteWishlistRepository::add(const domain::WishlistItem& item) {
    ...
    sqlite3_bind_text(stmt.get(), 7, item.image_url.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt.get(), 8, item.local_image_path.c_str(), -1, SQLITE_TRANSIENT);
    ...
    if (sqlite3_step(stmt.get()) != SQLITE_DONE) {
        Logger::instance().error(
            std::format("Failed to insert wishlist item: {}", sqlite3_errmsg(db.getHandle()))
        );
        return -1;
    }
...
bool SqliteWishlistRepository::update(const domain::WishlistItem& item) {
    ...
    sqlite3_bind_text(stmt.get(), 6, item.image_url.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt.get(), 7, item.local_image_path.c_str(), -1, SQLITE_TRANSIENT);
    ...
    sqlite3_bind_text(stmt.get(), 11, last_checked_str.c_str(), -1, SQLITE_TRANSIENT);
...
std::optional<domain::WishlistItem> SqliteWishlistRepository::findByUrl(std::string_view url) {
    auto& db = DatabaseManager::instance();
    auto lock = db.lock();
...
domain::PaginatedResult<domain::WishlistItem> SqliteWishlistRepository::findAll(
...
domain::WishlistItem SqliteWishlistRepository::fromStatement(sqlite3_stmt* stmt) {
    ...
    if (const char* image_url = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 7))) {
        item.image_url = image_url;
    }
    if (const char* local_path = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 8))) {
    ...
    const char* last_checked = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 13));
...
std::chrono::system_clock::time_point SqliteWishlistRepository::stringToTimePoint(
