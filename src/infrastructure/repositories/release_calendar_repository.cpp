...
int SqliteReleaseCalendarRepository::add(const domain::ReleaseCalendarItem& item) {
    ...
    sqlite3_bind_text(stmt.get(), 2, release_date_str.c_str(), -1, SQLITE_TRANSIENT);
    ...
    sqlite3_bind_text(stmt.get(), 5, item.image_url.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt.get(), 6, item.local_image_path.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt.get(), 7, item.product_url.c_str(), -1, SQLITE_TRANSIENT);
    ...
    sqlite3_bind_text(stmt.get(), 12, created_at_str.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt.get(), 13, last_updated_str.c_str(), -1, SQLITE_TRANSIENT);

    if (sqlite3_step(stmt.get()) != SQLITE_DONE) {
        Logger::instance().error(
            std::format("Failed to insert release calendar item: {}", sqlite3_errmsg(db.getHandle()))
        );
        return -1;
    }
...
bool SqliteReleaseCalendarRepository::update(const domain::ReleaseCalendarItem& item) {
    auto& db = DatabaseManager::instance();
    auto lock = db.lock();
    ...
    sqlite3_bind_double(stmt.get(), 10, item.price);
    sqlite3_bind_text(stmt.get(), 11, item.notes.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt.get(), 12, last_updated_str.c_str(), -1, SQLITE_TRANSIENT);
...
std::optional<domain::ReleaseCalendarItem> SqliteReleaseCalendarRepository::findById(int id) {
    auto& db = DatabaseManager::instance();
    auto lock = db.lock();
...
std::optional<domain::ReleaseCalendarItem> SqliteReleaseCalendarRepository::findByUrl(std::string_view url) {
    auto& db = DatabaseManager::instance();
    auto lock = db.lock();

    auto stmt = db.prepare("SELECT * FROM release_calendar WHERE product_url = ?");
    sqlite3_bind_text(stmt.get(), 1, std::string(url).c_str(), -1, SQLITE_TRANSIENT);
...
std::vector<domain::ReleaseCalendarItem> SqliteReleaseCalendarRepository::findAll() {
    auto& db = DatabaseManager::instance();
    auto lock = db.lock();

    auto stmt = db.prepare("SELECT * FROM release_calendar ORDER BY release_date ASC");
...
domain::PaginatedResult<domain::ReleaseCalendarItem> SqliteReleaseCalendarRepository::findAll(
    ...
    auto stmt = db.prepare(
        "SELECT * FROM release_calendar ORDER BY release_date ASC LIMIT ? OFFSET ?"
    );
...
std::vector<domain::ReleaseCalendarItem> SqliteReleaseCalendarRepository::findByDateRange(
    ...
    auto stmt = db.prepare(
        "SELECT * FROM release_calendar WHERE release_date >= ? AND release_date <= ? ORDER BY release_date ASC"
    );
...
int SqliteReleaseCalendarRepository::removeOlderThan(const std::chrono::system_clock::time_point& cutoff_date) {
    auto& db = DatabaseManager::instance();
    auto lock = db.lock();
...
domain::ReleaseCalendarItem SqliteReleaseCalendarRepository::fromStatement(sqlite3_stmt* stmt) {
    domain::ReleaseCalendarItem item;

    item.id = sqlite3_column_int(stmt, 0);
    item.title = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1));

    const char* release_date = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2));
    item.release_date = stringToTimePoint(release_date);

    item.format = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3));

    if (const char* studio = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 4))) {
        item.studio = studio;
    }
    if (const char* image_url = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 5))) {
        item.image_url = image_url;
    }
    if (const char* local_path = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 6))) {
        item.local_image_path = local_path;
    }
    if (const char* product_url = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 7))) {
    ...
    if (const char* notes = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 11))) {
        item.notes = notes;
    }

    const char* created_at = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 12));
    const char* last_updated = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 13));
...
std::chrono::system_clock::time_point SqliteReleaseCalendarRepository::stringToTimePoint(
