...
std::string ConfigManager::get(std::string_view key, std::string_view default_value) const {
    auto value = get(key);
...
int ConfigManager::getInt(std::string_view key, int default_value) const {
    ...
    try {
        ...
        Logger::instance().warning(
            std::format("Failed to parse int config '{}': {}", key, *value)
        );
        return default_value;
    }
}

double ConfigManager::getDouble(std::string_view key, double default_value) const {
    auto value = get(key);
    if (!value) {
        return default_value;
    }
    ...
    try {
        ...
        Logger::instance().warning(
            std::format("Failed to parse double config '{}': {}", key, *value)
        );
        return default_value;
    }
}
...
void ConfigManager::set(std::string_view key, std::string_view value) {
    ...
    sqlite3_bind_text(stmt.get(), 1, std::string(key).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt.get(), 2, std::string(value).c_str(), -1, SQLITE_TRANSIENT);

    if (sqlite3_step(stmt.get()) != SQLITE_DONE) {
        Logger::instance().error(
            std::format("Failed to set config '{}': {}", key, sqlite3_errmsg(db.getHandle()))
        );
    }
}
