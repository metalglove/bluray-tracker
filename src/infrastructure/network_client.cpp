...
NetworkClient::NetworkClient(NetworkClient &&other) noexcept
    : curl_(other.curl_), user_agent_(std::move(other.user_agent_)),
      timeout_(other.timeout_), follow_redirects_(other.follow_redirects_) {
  other.curl_ = nullptr;
}
...
HttpResponse NetworkClient::get(std::string_view url,
  ...
  if (res != CURLE_OK) {
    Logger::instance().error(fmt::format("GET request failed for {}: {}", url,
...
HttpResponse NetworkClient::post(std::string_view url,
  ...
  if (res != CURLE_OK) {
    Logger::instance().error(fmt::format("POST request failed for {}: {}", url,
                                         curl_easy_strerror(res)));
    return response;
  }
...
NetworkClient::downloadFile(std::string_view url) {
  ...
  if (res != CURLE_OK) {
    Logger::instance().error(fmt::format("File download failed for {}: {}", url,
                                         curl_easy_strerror(res)));
    return std::nullopt;
  }
  ...
  if (status_code < 200 || status_code >= 300) {
    Logger::instance().error(fmt::format(
        "File download failed with status {}: {}", status_code, url));
    return std::nullopt;
  }
...
size_t NetworkClient::writeCallback(void *contents, size_t size, size_t nmemb,
                                    void *userp) {
