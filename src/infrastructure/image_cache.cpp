...
ImageCache::ImageCache(std::string_view cache_directory)
    ...
    if (!std::filesystem::exists(cache_dir_)) {
        ...
        Logger::instance().info(
            std::format("Created cache directory: {}", cache_dir_.string())
        );
    }
}
...
std::optional<std::string> ImageCache::cacheImage(std::string_view image_url) {
    ...
    if (!image_data || image_data->empty()) {
        Logger::instance().warning(
            std::format("Failed to download image: {}", image_url)
        );
        return std::nullopt;
    }
    ...
    Logger::instance().debug(
        std::format("Cached image: {} -> {}", image_url, file_path.string())
    );

    return file_path.string();
}

std::optional<std::string> ImageCache::getCachedPath(std::string_view image_url) const {
    if (image_url.empty()) {
        return std::nullopt;
    }
...
void ImageCache::clear() {
    ...
    Logger::instance().info(std::format("Cleared {} cached images", count));
}
...
std::string ImageCache::generateFilename(std::string_view url) const {
    ...
    SHA256(
        reinterpret_cast<const unsigned char*>(url.data()),
        url.size(),
        hash
    );
...
std::string ImageCache::detectExtension(std::string_view url) const {
    ...
    if (dot_pos != std::string_view::npos) {
        size_t end_pos = query_pos != std::string_view::npos ? query_pos : url.size();
