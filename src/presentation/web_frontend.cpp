...
void WebFrontend::setupActionRoutes() {
  // Trigger scrape now
  CROW_ROUTE(app_, "/api/scrape").methods("POST"_method)([this]() {
...
<body data-theme="dark">
    ...
    <script>
        ...
        async function viewPriceHistory(id) {
            ...
            // Show simple loading state on canvas if needed, or just wait
            ...
            try {
                ...
                renderChart(ctx, history, item.desired_max_price);
            } catch (error) {
                console.error(error);
        ...
        function renderChart(ctx, history, targetPrice) {
            const labels = history.map(h => {
                const date = new Date(h.date);
                return date.toLocaleDateString() + ' ' + date.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
            });
            const prices = history.map(h => h.price);
            const stock = history.map(h => h.in_stock);
                        ...
                        tooltip: {
                            callbacks: {
                                afterLabel: function(context) {
                                    if (context.datasetIndex === 0) { // Price dataset
                                        const index = context.dataIndex;
                                        return stock[index] ? 'In Stock' : 'Out of Stock';
                                    }
                                }
                            }
                        }
                ...
                <tr>
                    ...
                    <td>
                        <div style="display: flex; gap: 0.5rem;">
                            <button class="btn btn-info" style="padding: 0.5rem 0.75rem;" onclick="viewPriceHistory(${item.id})" title="View History">📈</button>
